...
var speed = 0.001;                        // Wave animation speed
var wavePoints = 5;       
var simplex = new SimplexNoise();// How many point will be used to compute our wave

var points = [];

function calculateWavePoints(asd) {
  var points = [];
  ...
  for (var i = 0; i <= wavePoints; i++) {
    ...
    var factor = simplex.noise2D(x , asd);
    ...
    points.push({x: x , y: (factor * waveDelta) + waveHeight});
  }
...
function tick() {
  ...
  if (lastUpdate) {
    ...
    wave3.setAttribute('d', buildPath(calculateWavePoints((frame3 = frame3 + 0.008) + 2400)));
  } else {
    lastUpdate = now;
  }
